...
Для своего названия в статусе Steam остановите аккаунт и откройте **Настроить → 🎮 Кастомная игра**. Введите одну строку до 64 символов и снова запустите аккаунт. Бот передаст название как стороннюю игру первым элементом, затем все настроенные App ID в прежнем порядке. Название хранится в SQLite отдельно для каждого аккаунта; кнопка **Убрать кастомную игру** возвращает обычный список.

Это статус стороннего приложения (Non-Steam Game), а не запуск `.exe` или переименование игр в библиотеке. Видимость статуса зависит от приватности Steam. [О сторонних играх в Steam](https://help.steampowered.com/en/faqs/view/4B8B-9697-2338-40EC).
